...
Network scripts detect the active route interface by default. Override with `DIONYSUS_NET_IFACE` when needed.
VPN detection checks NordVPN when available, then common VPN interface prefixes such as `tun`, `tap`, `wg`, `tailscale`, and `zt`.
